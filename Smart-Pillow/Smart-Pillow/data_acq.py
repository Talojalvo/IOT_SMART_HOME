...
def init_db(database):
    # database = r"data\Smart-PillowData.db"    
    # 
    #
    # - here we deleted addres an 
    # + we added Smart-PillowCardId 
    tables = [
...
def csv_acq_data(table_name):
        ...
        try:
            if db_init:                
                data = pd.read_csv("data/Smart-PillowData.csv")
...
def create_IOT_dev(name, status, units, last_updated, update_interval, Smart-PillowCarId, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special):
    """
    ...
    :return: sys_id
    """
    sql = ''' INSERT INTO iot_devices(name, status, units, last_updated, update_interval, Smart-PillowCarId, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special)
              VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
    conn = create_connection()
    if conn is not None:
        cur = conn.cursor()
        cur.execute(sql, [name, status, units, last_updated, update_interval, Smart-PillowCarId, placed, dev_type, enabled, state, mode, fan, temperature, dev_pub_topic, dev_sub_topic, special])
        conn.commit()
        re = cur.lastrowid
        conn.close()
        return re
...
if __name__ == '__main__':
    if db_init:
        ...
        numb =create_IOT_dev('alarm', 'off', 'N', timestamp(), 300, 'Smart-PillowCarId', 'left front', 'alarm', 'false', 'cooling', 'mode', 'fan', '32', comm_topic+'ala-1/pub', comm_topic+'ala-1/sub', 'changed')
        numb =create_IOT_dev('DHT-1', 'on', 'celcius', timestamp(), 300, 'Smart-PillowCarId', 'placed', 'detector', 'enabled', 'state', 'mode', 'fan', 'temperature', comm_topic+'DHT-1/pub', comm_topic+'DHT-1/sub', 'done')
        numb =create_IOT_dev('SensitivityMeter', 'on', 'N', timestamp(), 3600, 'Smart-PillowCarId', 'placed', 'meter', 'enabled', 'state', 'mode', 'fan', 'NA', comm_topic+'SensitivityMeter/pub', comm_topic+'SensitivityMeter/sub', 'done')
        numb =create_IOT_dev('ElecMeter', 'on', 'kWh', timestamp(), 3600, 'Smart-PillowCarId', 'placed', 'meter', 'enabled', 'state', 'mode', 'fan', 'NA', comm_topic+'elecMeter/pub', comm_topic+'elecMeter/sub', 'done')
        numb =create_IOT_dev('Motion', 'off', 'km', timestamp(), 600, 'Smart-PillowCarId', 'placed', 'actuator-detector', 'enabled', 'state', 'mode', 'fan', '85', comm_topic+'motion/pub', comm_topic+'motion/sub', 'done')
